...
const HomeScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const [selectedTab, setSelectedTab] = useState<'recent' | 'shared' | 'favorite' | 'template'>('recent');
  ...
  const loadData = async () => {
    try {
      const [allPrompts, allCategories, recentPrompts, favoritePrompts, metrics] = await Promise.all([
        ...
        PromptManager.getRecentPrompts(10),
        PromptManager.getFavoritePrompts(),
  ...
  const handleCopyPrompt = async (promptId: string) => {
    const success = await PromptManager.copyPromptToClipboard(promptId);
    if (success) {
      Alert.alert('成功', 'プロンプトをクリップボードにコピーしました');
      // Refresh recent prompts since last accessed time changed
      const updated = await PromptManager.getRecentPrompts(10);
      ...
      <View style={styles.header}>
        <View style={styles.headerContent}>
          <View style={styles.logoSection}>
            ...
            <View>
              <Text style={styles.appTitle}>PromptStocks</Text>
              <Text style={styles.appSubtitle}>AIプロンプトライブラリ</Text>
            </View>
          </View>
      ...
      >
        ...
        <Card style={styles.statsCard} variant="glass">
          <View style={styles.statsHeader}>
            <Text style={styles.statsTitle}>ニューラルストレージ</Text>
          ...
          <View style={styles.statsGrid}>
            ...
            <View style={styles.statItem}>
              <Text style={[styles.statValue, { color: '#8B5CF6' }]}>{storageStats.categoryCount}</Text>
              <Text style={styles.statLabel}>カテゴリ</Text>
            </View>
            <View style={styles.statItem}>
              <Text style={[styles.statValue, { color: '#F59E0B' }]}>{storageStats.totalSize}</Text>
        ...
        <View style={styles.promptsContainer}>
          {getDisplayPrompts().map((prompt, index) => (
            <Card
              key={prompt.id}
              style={[styles.promptCard, ...(index === 0 ? [styles.featuredCard] : [])]}
                ...
                <View style={styles.promptMeta}>
                  ...
                  {index === 0 && (
                    <View style={styles.featuredBadge}>
                      <Text style={styles.badgeText}>注目</Text>
                    </View>
                  )}
                  <View style={styles.categoryBadge}>
                    <Text style={styles.badgeText}>作業効率</Text>
                  </View>
                </View>
