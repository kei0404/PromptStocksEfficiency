...
class AsyncStorageService {
  ...
  async savePrompt(prompt: Prompt): Promise<void> {
    try {
      ...
      this.cache.delete(`prompts:category:${prompt.category}`);
    } catch (error) {
  ...
  async getAllPrompts(): Promise<Prompt[]> {
    try {
      ...
      const processedPrompts = prompts.map(prompt => ({
        ...
        lastUsedAt: new Date(prompt.lastUsedAt)
      }));
  ...
  async getPromptsByCategory(category: string): Promise<Prompt[]> {
    try {
      const cacheKey = `prompts:category:${category}`;
      const cached = this.getCache<Prompt[]>(cacheKey);
      if (cached) return cached;

      const allPrompts = await this.getAllPrompts();
      const categoryPrompts = allPrompts.filter(prompt => prompt.category === category);
  ...
  async getStorageMetrics(): Promise<StorageMetrics> {
    try {
      ...
      const totalSize = allPrompts.reduce((sum, prompt) => sum + JSON.stringify(prompt).length, 0);
      const maxSize = allPrompts.reduce((max, prompt) => Math.max(max, JSON.stringify(prompt).length), 0);
  ...
  async getFavoritePrompts(): Promise<Prompt[]> {
    try {
      const allPrompts = await this.getAllPrompts();
      return allPrompts.filter(prompt => prompt.isFavorite && !prompt.isArchived);
    } catch (error) {
  ...
  async getMostUsedPrompts(limit: number = 10): Promise<Prompt[]> {
    try {
      ...
      console.error('Failed to get most used prompts:', error);
      return [];
    }
  }
